...
abstract class UpManga : KeiSource() {
    ...
    private fun parseMangaDetails(document: Document) = SManga.create().apply {
        ...
        author = document.infoValue("ผู้แต่ง", "Author")
        artist = document.infoValue("ผู้เขียน", "Artist")

        val genres = document.select(".seriestugenre a").map { it.text() }.toMutableList()
        document.infoValue("ประเภท", "Type")?.let(genres::add)
        if (document.selectFirst(".thumb .colored") != null) genres.add("Color")
        genre = genres.distinct().joinToString()

        status = when (document.infoValue("สถานะ", "Status")?.lowercase()) {
        ...
        val alternative = document.selectFirst(".seriestualt")?.text()
    ...
    private fun Document.infoValue(vararg labels: String): String? = select(".infotable tr")
        .firstOrNull { row ->
            row.selectFirst("td")?.text()?.let { label -> labels.any { it.equals(label, ignoreCase = true) } } == true
        }
        ?.select("td")
        ?.getOrNull(1)
        ?.text()
    ...
    companion object {
        ...
        private val dateFormat = DateTimeFormatter.ofPattern("MMMM d, uuuu", Locale.forLanguageTag("th"))
    }
}
